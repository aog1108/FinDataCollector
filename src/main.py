...
if batch_name == 'SOD':
    ...
    raise RuntimeError('Invalid batch type')
...
if len(sys.argv) == 1 and today.weekday() in (0, 6) and batch_config_name == 'SODBatchConfig':
    batch_handler.set_batch_function_flag('do_equity_index_data_batch', 'Off')

if len(sys.argv) == 1 and today.weekday() in (5, 6) and batch_config_name == 'EODBatchConfig':
    sys.exit()
...
# batch_handler.set_batch_function_flag('do_update_krx_listed_stocks_info_batch', 'On')
